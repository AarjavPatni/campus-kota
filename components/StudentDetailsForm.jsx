  ...
  const handleValidationSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("student_mobile", validationData.phone);

      if (error) throw error;

      if (data && data.length > 0) {
        setToastMessage({
          text: "A student with this phone number already exists!",
          type: "error",
        });
        setToggleToast(true);
        setToastOpacity(1);
      ...
      // If phone number doesn't exist, proceed to main form
      setShowValidationStep(false);
      
      // Pre-fill the form with the validation data
      const firstName = validationData.first_name;
      
      formik.setValues({
        ...formik.values,
        first_name: firstName,
        student_mobile: validationData.phone,
      });
    } catch (error) {
      setToastMessage({
        text: "Error checking phone number: " + error.message,
        type: "error",
      });
      setToggleToast(true);
      setToastOpacity(1);
              ...
              <form onSubmit={handleValidationSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    id="first_name"
                    value={validationData.first_name}
                    onChange={(e) =>
                      setValidationData({ ...validationData, first_name: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setValidationData({ ...validationData, phone: e.target.value })
                    }
                    className="w-full p-2 bg-gray-800 text-white rounded"
                    required
                  />
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 bg-gray-800 text-white rounded"
                  />
                  {formik.touched.original_room && formik.errors.original_room && (
                    <div className="text-red-500 text-sm mt-1">
                      {formik.errors.original_room}
                    </div>
                  )}
                </div>
                <div>
